...
#
#
# Modifications copyright (C) 2017, imec/Matthias De Geyter
# (added additional options to report docker stats)
...
Usage: mon-put-instance-data.pl [options]
  ...
  Adapted to also sample docker stats. (mem, cpu percentage)

  There is a limit of 20 metrics per request, so if you have lots of docker containers
  running, make sure to make separate calls for mem & cpu in docker
...
Description of available options:
  ...
  --docker-mem        Reports docker stats (mem %) for running containers
  --docker-cpu        Reports docker stats (cpu %) for running containers
...
my $report_docker_mem;
my $report_docker_cpu;
my @mount_path;
my $mem_units;
my $disk_units;
my $mem_unit_div = 1;
...

{
  ...
  $parse_result = GetOptions(
    ...
    'docker-mem' => \$report_docker_mem,
    'docker-cpu' => \$report_docker_cpu,
...
if (!$report_mem_util && !$report_mem_used && !$report_mem_avail
  && !$report_swap_util && !$report_swap_used && !$report_disk_space && !$report_docker_mem
  && !$report_docker_cpu)
{
...
sub add_single_metric
{
  ...
  ++$mcount;
  # there is a limit of 20 MetricDatums/PutMetricData
...
sub add_metric
{
  ...
  $xdims{'ContainerName'} = $container_name if $container_name;
...
if ($report_docker_cpu || $report_docker_mem)
{
  my @stats = readpipe('docker stats --format "{{.Name}}:{{.MemPerc}}:{{.CPUPerc}}" --no-stream');
  foreach my $line (@stats)
  {
    my @fields = split(':', $line);
    my $container_name = $fields[0];
    my $mem_perc = substr($fields[1], 0, index($fields[1], '%'));
    my $cpu_perc = substr($fields[2], 0, index($fields[2], '%'));
    if ($report_docker_cpu) {
      add_metric('ContainerCpuUtilization', 'Percent', $cpu_perc, undef, undef, $container_name);
    }
    if ($report_docker_mem) {
      add_metric('ContainerMemoryUtilization', 'Percent', $mem_perc, undef, undef, $container_name);
    }
  }
}
